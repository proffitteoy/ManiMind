...
def build_context_blueprint(project_id: str) -> list[ContextRecord]:
    return [
        ...
        ContextRecord(
            ...
            summary="片段资产索引与交付清单",
            writer_role="lead",
            consumer_roles=["reviewer", "human_reviewer", "lead"],
            lifecycle="project",
            invalidation_rule="rebuild_or_post_produce_retry",
            sticky=True,
        ),
        ContextRecord(
            key=f"{project_id}.review.evidence",
            scope=ContextScope.LONG_TERM,
            summary="审核阶段结构化证据（产物路径、日志与检查点）",
            writer_role="lead",
            consumer_roles=["reviewer", "human_reviewer", "lead"],
            lifecycle="project",
        ...
        ContextRecord(
            ...
            scope=ContextScope.LONG_TERM,
            summary="人工确认后的审核结论与阻塞项",
            writer_role="human_reviewer",
            consumer_roles=["lead"],
            lifecycle="project",
            invalidation_rule="next_review_round",
            sticky=True,
        ...
        ContextRecord(
            key=f"{project_id}.review.return.memo",
        ...
        ContextRecord(
            key=f"{project_id}.review.return.prompt_patch",
            scope=ContextScope.SHORT_TERM,
            summary="人工打回后注入下一轮执行提示词的返工指令",
            writer_role="human_reviewer",
            consumer_roles=[
                "lead",
                "coordinator",
...
def build_agent_profiles(
    ...
    return [
        ...
        AgentProfile(
            ...
            responsibility="只消费结构化产物并输出审核草案，不直接放行。",
            allowed_stages=[PipelineStage.REVIEW],
            required_inputs=shared_inputs + worker_outputs + [f"{project_id}.review.evidence"],
            owned_outputs=[],
            output_contract="只能输出审核草案，最终放行由人工确认。",
        ),
        AgentProfile(
            id="human_reviewer",
            mode=AgentMode.VERIFY_ONLY,
            responsibility="人工确认审核草案，给出 approve 或 return 的最终结论。",
            allowed_stages=[PipelineStage.REVIEW],
            required_inputs=shared_inputs + worker_outputs + [f"{project_id}.review.evidence"],
            owned_outputs=[
                f"{project_id}.review.report",
                f"{project_id}.review.return.memo",
                f"{project_id}.review.return.prompt_patch",
            ],
            output_contract="必须给出最终人工审核结论，未确认前不得进入后处理。",
        ),
    ]
...
def build_execution_tasks(
    ...
    tasks = [
        ExecutionTask(
            ...
            blocks=["explore.references"],
            required_outputs=[f"{project_id}.session.handoff"],
        ),
        ExecutionTask(
            id="explore.references",
            subject="探索输入材料并提取候选事实与风险点",
            owner_role="explorer",
            active_form="正在探索论文与笔记引用",
        ...
        ExecutionTask(
            ...
            stage=PipelineStage.SUMMARIZE,
            blocked_by=["explore.references"],
            blocks=["plan.research_brief"],
            required_outputs=[
                f"{project_id}.research.summary",
                f"{project_id}.glossary",
                f"{project_id}.formula.catalog",
        ...
        ExecutionTask(
            id="plan.research_brief",
            subject="基于探索与总结生成结构化规划建议",
            owner_role="planner",
            active_form="正在生成规划建议",
        ...
        ExecutionTask(
            ...
            stage=PipelineStage.PLAN,
            blocked_by=["plan.research_brief"],
