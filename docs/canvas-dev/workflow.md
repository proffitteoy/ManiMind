...
ManiMind 当前约定：
- 正式白板文件固定为 `docs/architecture.canvas`
- 当前粒度固定为“模块/子系统级”
- 每次结构变更必须同步更新白板与 Markdown 架构文档

1. 理解核心理念：Canvas 白板作为架构主视图，代码是其可执行映射；图形语言优于纯文字描述；人类负责架构设计，AI负责代码实现
/
...
/
6. 建立代码-白板同步机制：如果没有自动化工具，就把“结构改动后同步更新 `docs/architecture.canvas`”作为强制人工步骤；模块新增/删除、依赖方向变化、状态路径变化都必须进白板
/
...
/
11. 维护白板与代码一致性：每次 PR/MR 合并前检查白板是否需要更新；至少同步检查 `docs/architecture.canvas` 和 `docs/通用项目架构模板.md`；发现不一致时必须修正文档，并判断是否需要同步调整代码
/
